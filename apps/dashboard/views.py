...
class LeadBrokerDashboardView(LoginRequiredMixin, EmailVerifiedRequiredMixin, View):
    """Lead Broker dashboard — KPI cards + charts from REAL Lead/Sale data.

    Mirrors the "CRM Lead Broker" design (click, lead, revenue, conversion
    rate + 3 charts + activity feed) but reads from the actual DB instead
    of mock data. Lives at "/" so post-login lands here.
    """
    ...
    def get(self, request):
        ...
        # ── KPI cards ────────────────────────────────────────────────
        leads_total_month = Lead.objects.filter(created_at__date__gte=month_start).count()
        leads_total_prev_month = Lead.objects.filter(
        ...
        sales_this_month = Sale.objects.filter(created_at__date__gte=month_start)
        sold = sales_this_month.filter(status=Sale.STATUS_SOLD)
        revenue_month = sold.aggregate(total=Sum("product__price"))["total"] or Decimal("0")
        conv_rate = (
            sold.count() * 100 / sales_this_month.count()
        ) if sales_this_month.count() else 0

        def trend(curr, prev):
            if not prev:
                return ("+0.0%", "up") if curr else ("0.0%", "flat")
            delta = (curr - prev) * 100 / prev
            return (f"{delta:+.1f}%", "up" if delta >= 0 else "down")

        leads_trend = trend(leads_total_month, leads_total_prev_month)
        kpis = [
            {"label": "Lead totali (mese)", "value": f"{leads_total_month:,}",
             "delta": leads_trend[0], "trend": leads_trend[1],
             "icon": "mouse-pointer",
             "gradient": "from-indigo-500 to-purple-600"},
            {"label": "Lead oggi", "value": f"{leads_today}",
             "delta": "", "trend": "up", "icon": "user-plus",
             "gradient": "from-pink-500 to-rose-500"},
            {"label": "Revenue (mese)", "value": f"€{revenue_month:,.0f}",
             "delta": "", "trend": "up", "icon": "dollar-sign",
             "gradient": "from-sky-500 to-cyan-500"},
            {"label": "Conversion rate", "value": f"{conv_rate:.1f}%",
             "delta": "", "trend": "up", "icon": "target",
             "gradient": "from-emerald-500 to-teal-500"},
        ]

        # ── Chart: leads per source (last 7 days, bar chart) ─────────
        by_source = (
            Lead.objects.filter(created_at__date__gte=week_start)
            .values("source")
            .annotate(n=Count("id"))
            .order_by("-n")[:6]
        )
        broker_chart = {
            "labels": [row["source"] or "—" for row in by_source],
            "data": [row["n"] for row in by_source],
        }

        # ── Chart: leads by status (pie / doughnut) ──────────────────
        by_status = (
        ...
        status_chart = {
            "labels": [row["status"] or "lead" for row in by_status],
            "data": [row["n"] for row in by_status],
        }

        # ── Chart: leads grouped by country (last 30 days, doughnut) ─
        last_30 = today - timedelta(days=29)
        by_country = (
            Lead.objects.filter(created_at__date__gte=last_30)
            .values("country")
            .annotate(n=Count("id"))
            .order_by("-n")[:5]
        )
        country_chart = {
            "labels": [(row["country"] or "—").upper() for row in by_country],
            "data": [row["n"] for row in by_country],
        }

        # ── Activity feed (last 8 events: sales status + new leads) ──
        activities = []
        for s in Sale.objects.select_related("product").order_by("-updated_at")[:4]:
            if s.status == Sale.STATUS_SOLD:
                icon, tone = "check", "success"
                text = f"FTD su {s.product.name}"
                extra = f"€{s.product.price}" if s.product.price else "—"
            elif s.status == Sale.STATUS_LOST:
                icon, tone = "x", "danger"
                text = f"Mancato acquisto — {s.product.name}"
                extra = ""
            else:
                icon, tone = "clock", "warning"
                text = f"In attesa — {s.product.name}"
                extra = ""
            activities.append({
                "icon": icon, "tone": tone,
        ...
        for ld in Lead.objects.order_by("-created_at")[:4]:
            activities.append({
                "icon": "user-plus", "tone": "info",
                "title": ld.full_name or ld.email or f"Lead {ld.pk}",
                "text": "Nuovo lead",
                "extra": ld.source or "",
                "when": ld.created_at,
            })
