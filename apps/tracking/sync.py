...
def sync_zenvior(broker, days=90, only_ids=None):
    """Pull stati Zenvior: GET /api/affiliate/leads?from=&to=&page=&limit=
    (paginata, envelope {leads:[...], page, pages, total}). Aggancio via
    _match_lead sul click_id (campo esplicito nella risposta). FTD =
    has_conversion:true (zenvior.is_deposit), NON nelle chiavi generiche
    _DEPOSIT_KEYS -> calcolata a parte."""
    from . import zenvior
    now = datetime.now(dt_tz.utc)
    date_start = (now - timedelta(days=days)).strftime("%Y-%m-%d")
    date_end = now.strftime("%Y-%m-%d")
    ...
    while page <= MAX_PAGES:
        try:
            rows, pages = zenvior.pull_leads(broker, date_start, date_end, page=page)
        except zenvior.ZenviorError:
            break
        for row in rows:
            if not isinstance(row, dict):
                continue
...
def sync_all_pullable():
    ...
    jobs = ([(b, sync_broker) for b in TrackboxBroker.objects.filter(is_active=True)]
            ...
            + [(b, sync_zenvior) for b in ZenviorBroker.objects.filter(is_active=True)]
...
def sync_selected(lead_ids):
    ...
    for b in brokers.values():
        try:
            ...
            elif isinstance(b, ZenviorBroker):
                r = sync_zenvior(b, only_ids=ids)
